...
!	bugs and notes:
!	addwt probably should only use pixels within extint
...
!	open script and log files
        ...
        read '(a8)', yn
        if(index(yn,'n')>0) then
          open(unit=iupl,file=pipelog,access='append')
        else
          open(unit=iupl,file=pipelog)
        endif
...
!            lskrot = krot
    ...
    8     pval = line(ival:79)
          ...
          elseif(index(pname,'dofits')>0) then
            if((index(pval,'false')>0).or.(index(pval,'none')>0)) then
              dofits = .false.
          ...
          elseif(index(pname,'rdfits')>0) then
            if(index(pval,'false')>0) then
              rdfits = .false.
            elseif(index(pval,'true')>0) then
              rdfits = .true.
            else
...
!	note: can''t just use old flat since it is torted
              if(.not.testrun) modecard = -modecard
...
!	will be changed in lo-res mode
          ...
          elseif(index(pname,'wno')>0) then
            ...
            if((modecard==MNONE).or.(modecard==MBLK)) then
              wno0 = abs(wno0)
            else
              wno0 = -wno0
            endif
...
!	crossdisp may change when a file is read, but this shouldn't hurt
        ...
        elseif(index(line,'.')>0) then
          ...
          if((checkall.or.(checkwno.and.(fname/=fnold))) &
            .and.(modecard/=MNONE).and.(modecard/=MBLK)) &
            wno0 = -abs(wno0)
          fnold = fname
...
!	note: dist is now in pixels
        ...
        if(nodon.and.(abs(slitpa)<360.) &
          ...
          print '("*** nodpa /= slitpa ",2f8.2)', nodpa,-slitpa
          if(askpa) then
...
!	open raw, reduced, and fits header files
        ...
        if(ifobj) then
          ...
          if(ierr/=0) then
            ...
            print '(" Skipping file.  Hit RETURN to continue or q to quit",$)'
            read '(a8)', yn
          ...
          if(leftopen) then
!            if(hdcopy) then
!              print '("*** Reduced header was left open")'
!            endif
            close(unit=iuredh)
          endif
          if(hdcopy) then
...
!	read, write, and parse lines
  ...
  100   read(unit=iurawh,fmt='(a79)',err=190,end=200) line
        ...
        elseif(line(1:8)=='obsmode ') then
          ...
          if(dofits) call fithchar('OBSMODE ',obsmode,NOC,iufith)
...
!              lskrot = .012
        ...
        elseif(line(1:8)=='pressure') then
          read(unit=line,fmt='(10x,f5.0)',err=190,end=190) pressure
...
!	no longer included in header
  ...
  110     if(date==0.) then
        ...
        elseif(line(1:8)=='ha      ') then
          ...
          comment = 'degrees'
          if(dofits) call fithreal('HA      ',ha,comment,iufith)
...
!              3f10.2)') echelle,wno,waveno0
!            if(doplot.and.(abs(wno-abs(waveno0))>wno/200.) &
!              .and.(modeobs<MNOD)) then
!              print '(" Adjust mr0? (n) ",$)'
!              read '(a8)', yn
!              if(index(yn,'y')>0) then
!                xdorder = iorder
...
!            else
!              print '("*** xdr may be wrong in tort")'
!            endif
          endif
          if(lores<0.) then
            print '("*** Warning: lores and instmode disagree")'
...
!	open cardfile and cardfits
        ...
        docardfits = dofits.and.(index(cardfits,'flt.fits')/=1)
        nz = nwrite*nc
...
!	if(dofits) copy rdfile to rawfits
...
!	new method to find xorder1
        ...
        print '(" New method xorder1,norder = ",f6.2,i6)', xorder0,norder0
        if(neworder) then
          xolder1 = xorder1
          nolder = norder
          xorder1 = xorder0
...
!        atmodir = '/home/lacy/pipe/'	! duck
!        atmodir = '/Users/lacy/Mirror/progs/atmo/'	! Mac
...
!	read atmo file and interpolate onto spec wno grid
          ...
          if(jerr/=0) then
            ...
            print '(" May need to change atmodir in fife.F90")'
...
!	check slitrot
          ...
          enddo
          ddids(2) = dids
          if(abs(ddids(1)-ddids(3))>0.2) then
            print '("*** Shift along slit: ",3f10.3)', &
...
!	long-slit or untorted data
          if(firstfile) then
            ...
            if(dofits) then
              ...
              comment = 'summed time per point (s)'
              call fithreal('SUMTIME ',sumtime,comment,iufhd)
              bunits = 'erg/(s cm2 sr cm-1)'
              comment = 'cgs intensity units'
          ...
          do ix = 1,nx
            ...
            enddo   
            if(nsum>0) then
              freq(ix,3) = stdsum/(sqrtn*nsum)
              freq(ix,4) = cardsum/nsum
              black(ix) = blksum/nsum
            else
              freq(ix,3) = 0.
              freq(ix,4) = 0.
              black(ix) = 0.
...
!	calculate first four lines
          do iorder = 1,norder
            ...
            do iy = 1,ny
              ...
              enddo   
              if(nsum>0) then
                freq(is,3) = stdsum/(sqrtn*nsum)
                freq(is,4) = cardsum/nsum
                black(is) = blksum/nsum
              else
                freq(is,3) = 0.
                freq(is,4) = 0.
                black(is) = 0.
...
!          plot(is,3) = blksum/max(1,nsum)
          plot(is,3) = specsum(is,3)
          plot(is,4) = specsum(is,4)
          plot(is,5) = blksum/max(1,nsum)
        enddo   
        scale = -1.
        if(ask) scale = 0.
        nplt = norder
