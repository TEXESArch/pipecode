...
!       read, write, and parse lines
  ...
  100   read(unit=iurawh,fmt='(a79)',err=190,end=200) line
        ...
        elseif(line(1:8)=='order   ') then
          ...
          call fithint('ORDER   ',lorder,comment,iufith)
...
!       may have to convert pixelwd to cm here
  ...
  110     date = iyr+(imo+idy/100.)/100.
        ...
        elseif(line(1:8)=='focus   ') then
          read(unit=line,fmt='(10x,f7.3)',err=191,end=191) focus
          call fithreal('FOCUS   ',focus,NOC,iufith)
        elseif(line(1:8)=='endtime ') then
          call fithchar('ENDTIME ',line(11:70),NOC,iufith)
...
!	interpret and write out to fits.hd
        ...
        real nodpa,lores,kmirror,krot
        character(8) yn
        logical ifobj,ifopen,cpline,doplot
        ...
        inquire(unit=iufith,opened=ifopen)
        if(ifopen) close(unit=iufith)
        open(unit=iufith,file='fits.hd')
...
!	interpret record as 36 80-character lines
        do ichar = 1,2880,80
          ...
          if(line(1:8)=='END     ') goto 120
          if(cpline) write(iufith,'(a80)') line
...
!	first 10 parameters not currently stored in common
          ...
          elseif(line(1:8)=='FILENAME') then
            if(index(val,'flat')==0) cpline = .true.
            if(cpline) write(iufith,'(a80)') line
...
!            if(wno0<=400.) wno0 = 1000.
          elseif(line(1:8)=='ORDER   ') then
            read(unit=val,fmt=*,err=190,end=190) xdorder
            lorder = nint(xdorder)
...
!       may have to convert pixelwd to cm here
          ...
          elseif(line(1:8)=='DATE-REL') then
            continue
          elseif(line(1:8)=='DATE    ') then
          ...
          elseif(line(1:8)=='DATE-OBS') then
!	I don''t know why this doesn't work, but I already know date
!            read(unit=val,fmt='(i4,1x,i2,1x,i2)',iostat=ierr) &
!              iyr,imo,idy
!            if(ierr==0) then
!              iyr = iyr-2000
...
!            endif
          ...
          elseif(line(1:8)=='EQUINOX ') then
            read(unit=val,fmt=*,err=191,end=191) equinox
          elseif(line(1:8)=='INSTRPA ') then
...
!        efl = efl0/(forerat*fred)
        ...
        if((modeinst==MHIMED).or.(modeinst.eq.MMED)) then
          ...
          elseif((abs(wno-abs(waveno0))>wno/100.) &
            ...
            print '("^G*** Warning: echelle and waveno0 disagree", &
...
!	leave open to write data
        ...
        close(unit=iufith)
        open(unit=iufith,file='fits.hd')
!	I could have just opened to append
        notdone = .true.
        do
